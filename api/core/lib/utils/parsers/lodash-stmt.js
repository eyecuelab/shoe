...
matchStatement = function matchStatement(statement, match) {
  return _.every(match, (value, key) => {
    if (_.isRegExp(value)) {
...
eachStatement = function eachStatement(statements, func, groupFunc) {
  _.each(statements, (statement, index) => {
...
findStatement = function findStatement(statements, match, keys) {
  return _.some(statements, (statement) => {
    if (_.has(statement, 'group')) {
...
rejectStatements = function rejectStatements(statements, func) {
  return _.reject(statements, (statement, index) => {
    let result;
...
mergeStatements = function mergeStatements( /* statements */ ) {
  const statementsArray = _.reduce(arguments, (array, arg) => {
    if (!arg) {
      return array;
    }
...
replaceStatements = function replaceStatements(statements, match, func) {
  return _.map(statements, (statement) => {
    if (_.has(statement, 'group')) {
...
printStatements = function printStatements(statements, indent) {
  ...
  function print(indent /* strs */ ) {
    const strs = [_.fill(new Array(indent), ' ').join('')].concat(Array.prototype.slice.call(arguments, 1));
